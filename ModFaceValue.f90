...
contains
       ...
       do k=1,nK,2; do j=1,nJ,2
          if(  all(Used_GB(-1:2,j:j+1,k:k+1,iBlock)) .and. &
             ...
             call accurate_reschange3d(&
                  ...
                  Fine1_VII    = Primitive_VGI(:, 1,j:j+1,k:k+1,iGang)    ,&
                  Fine2_VII    = Primitive_VGI(:, 2,j:j+1,k:k+1,iGang)    ,&
       ...
       do k=1,nK,2; do j=1,nJ,2
          if(  all(Used_GB(nI-1:nI+2,j:j+1,k:k+1,iBlock)).and. &
             ...
             call accurate_reschange3d(&
                  ...
                  Fine1_VII    = Primitive_VGI(:,nI,j:j+1,k:k+1,iGang)      ,&
                  Fine2_VII    = Primitive_VGI(:,nI-1,j:j+1,k:k+1,iGang)    ,&
       ...
       do k=1,nK,2; do i=1,nI,2
          if(  all(Used_GB(i:i+1,-1:2,k:k+1,iBlock)) .and. &
             ...
             call accurate_reschange3d(&
                  ...
                  Fine1_VII    =    Primitive_VGI(:,i:i+1, 1,k:k+1,iGang)   ,&
                  Fine2_VII    =    Primitive_VGI(:,i:i+1, 2,k:k+1,iGang)   ,&
       ...
       do k=1,nK,2; do i=1,nI,2
          if(  all(Used_GB(i:i+1,nJ-1:nJ+2,k:k+1,iBlock)) .and. &
             ...
             call accurate_reschange3d(&
                  ...
                  Fine1_VII    = Primitive_VGI(:,i:i+1, nJ,k:k+1,iGang)     ,&
                  Fine2_VII    = Primitive_VGI(:,i:i+1, nJ-1,k:k+1,iGang)   ,&
       ...
       do j=1,nJ,2; do i=1,nI,2
          if(  all(Used_GB(i:i+1,j:j+1,-1:2,iBlock)) .and. &
             ...
             call accurate_reschange3d(&
                  ...
                  Fine1_VII    =    Primitive_VGI(:,i:i+1,j:j+1, 1,iGang)   ,&
                  Fine2_VII    =    Primitive_VGI(:,i:i+1,j:j+1, 2,iGang)   ,&
       ...
       do j=1,nJ,2; do i=1,nI,2
          if(  all(Used_GB(i:i+1,j:j+1,nK-1:nK+2,iBlock)) .and. &
             ...
             call accurate_reschange3d(&
                  ...
                  Fine1_VII    = Primitive_VGI(:,i:i+1,j:j+1, nK,iGang)  ,   &
                  Fine2_VII    = Primitive_VGI(:,i:i+1,j:j+1, nK-1,iGang),   &
  ...
  subroutine get_face_accurate1d(iSideIn, iBlock)
    ...
    case(1)
       call accurate_reschange1d(&
            Coarse2_V       = Primitive_VGI(:,-1,1,1,iGang)     ,&
            Coarse1_V       = Primitive_VGI(:, 0,1,1,iGang)     ,&
            Fine1_V         = Primitive_VGI(:, 1,1,1,iGang)     ,&
            Fine2_V         = Primitive_VGI(:, 2,1,1,iGang)     ,&
            CoarseToFineF_V = LeftState_VX(:, 1,1,1)     ,&
            FineToCoarseF_V =RightState_VX(:, 1,1,1)     ,&
            FineF_V         = LeftState_VX(:, 2,1,1))
    case(2)
       call accurate_reschange1d(&
            Coarse2_V       = Primitive_VGI(:,nI+2,1,1,iGang)   ,&
            Coarse1_V       = Primitive_VGI(:,nI+1,1,1,iGang)   ,&
            Fine1_V         = Primitive_VGI(:,nI  ,1,1,iGang)   ,&
            Fine2_V         = Primitive_VGI(:,nI-1,1,1,iGang)   ,&
    ...
    case(1)
       ...
       do j=1,nJ,2
          call accurate_reschange2d(&
               ...
               Coarse1_VI      = Primitive_VGI(:, 0,j-2:j+3,1,iGang)   ,&
               Fine1_VI        = Primitive_VGI(:, 1,j:j+1,1,iGang)     ,&
               Fine2_VI        = Primitive_VGI(:, 2,j:j+1,1,iGang)     ,&
    ...
    case(2)
       ...
       do j=1,nJ,2
          call accurate_reschange2d(&
               ...
               Coarse1_VI      = Primitive_VGI(:,nI+1,j-2:j+3,1,iGang) ,&
               Fine1_VI        = Primitive_VGI(:,nI  ,j:j+1,1,iGang)   ,&
               Fine2_VI        = Primitive_VGI(:,nI-1,j:j+1,1,iGang)   ,&
    ...
    case(3)
       ...
       do i=1,nI,2
          call accurate_reschange2d(&
               ...
               Coarse1_VI      = Primitive_VGI(:,i-2:i+3,0,1,iGang)    ,&
               Fine1_VI        = Primitive_VGI(:,i:i+1,1,1,iGang)      ,&
               Fine2_VI        = Primitive_VGI(:,i:i+1,2,1,iGang)      ,&
    ...
    case(4)
       ...
       do i=1,nI,2
          call accurate_reschange2d(&
               ...
               Coarse1_VI      = Primitive_VGI(:,i-2:i+3,nJ+1,1,iGang) ,&
               Fine1_VI        = Primitive_VGI(:,i:i+1,nJ,1,iGang)     ,&
               Fine2_VI        = Primitive_VGI(:,i:i+1,nJ-1,1,iGang)   ,&
  ...
  contains
    ...
    subroutine calc_primitives(IArguments_I,iBlock)
      ...
      if(DoLimitMomentum)then
         ...
         BxFull = Primitive_VGI(Bx_,i,j,k,iGang)
         ByFull = Primitive_VGI(By_,i,j,k,iGang)
         BzFull = Primitive_VGI(Bz_,i,j,k,iGang)
         ...
         uBC2Inv= (Primitive_VGI(rhoUx_,i,j,k,iGang)*BxFull + &
              Primitive_VGI(rhoUy_,i,j,k,iGang)*ByFull + &
              Primitive_VGI(rhoUz_,i,j,k,iGang)*BzFull)*RhoC2Inv
         Ga2Boris= 1 + B2Full*RhoC2Inv

         Primitive_VGI(Ux_,i,j,k,iGang)= Primitive_VGI(rhoUx_,i,j,k,iGang)*&
              Ga2Boris - BxFull*uBC2Inv
         Primitive_VGI(Uy_,i,j,k,iGang)= Primitive_VGI(rhoUy_,i,j,k,iGang)*&
              Ga2Boris - ByFull*uBC2Inv
         Primitive_VGI(Uz_,i,j,k,iGang)= Primitive_VGI(rhoUz_,i,j,k,iGang)*&
              Ga2Boris - BzFull*uBC2Inv
      else
         Primitive_VGI(Ux_:Uz_,i,j,k,iGang) = &
              RhoInv*Primitive_VGI(RhoUx_:RhoUz_,i,j,k,iGang)
         do iFluid = 2, nFluid
            iRho = iRho_I(iFluid); iUx = iUx_I(iFluid); iUz = iUz_I(iFluid)
            RhoInv = 1/Primitive_VGI(iRho,i,j,k,iGang)
            Primitive_VGI(iUx:iUz,i,j,k,iGang) = &
                 RhoInv*Primitive_VGI(iUx:iUz,i,j,k,iGang)
         end do
      end if
      ...
      if(UseLogLimiter)then
         do iVar = 1, nVar
            if(UseLogLimiter_V(iVar)) Primitive_VGI(iVar,i,j,k,iGang) &
    ...
    subroutine get_facex_high(iMin, iMax, jMin, jMax, kMin, kMax, iBlock)
      ...
      if(TypeLimiter == 'no')then
         do k = kMin, kMax; do j = jMin, jMax; do i = iMin, iMax
            LeftState_VX(:,i,j,k) &
                 = c7over12 *(Primitive_VGI(:,i-1,j,k,iGang)  &
         ...
         do k = kMin, kMax; do j = jMin, jMax
            if(UseLowOrder) then
               Primitive_VI(:,iMin-nG:iMax+nG-1) &!dzy
                    = Primitive_VGI(:,iMin-nG:iMax+nG-1,j,k,iGang)
               if(nLowOrder==2) then
    ...
    subroutine get_facey_high(iMin, iMax, jMin, jMax, kMin, kMax, iBlock)
      ...
      if(TypeLimiter == 'no')then
         do k=kMin, kMax; do j=jMin, jMax; do i=iMin,iMax
            LeftState_VY(:,i,j,k) &
                 = c7over12*(Primitive_VGI(:,i,j-1,k,iGang) &
                 +           Primitive_VGI(:,i,j,k,iGang))  &
         ...
         do k = kMin, kMax; do i = iMin, iMax
            if(UseLowOrder) then
               Primitive_VI(:,jMin-nG:jMax+nG-1) &!dzy
                    = Primitive_VGI(:,i,jMin-nG:jMax+nG-1,k,iGang)
    ...
    subroutine get_facez_high(iMin, iMax, jMin, jMax, kMin, kMax, iBlock)
      ...
      if(TypeLimiter == 'no')then
         do k=kMin, kMax; do j=jMin, jMax; do i=iMin,iMax
            LeftState_VZ(:,i,j,k) &
                 = c7over12*(Primitive_VGI(:,i,j,k-1,iGang) &
                 +           Primitive_VGI(:,i,j,k,iGang))  &
         ...
         do j = jMin, jMax; do i = iMin, iMax
            ...
            if(UseLowOrder)then
               Primitive_VI(:,kMin-nG:kMax+nG-1) &!dzy
                    = Primitive_VGI(:,i,j,kMin-nG:kMax+nG-1,iGang)
    ...
    subroutine get_facex_first(iMin, iMax, jMin, jMax, kMin, kMax, iBlock)
      ...
      do k=kMin, kMax; do j=jMin, jMax; do i=iMin,iMax
         LeftState_VX(:,i,j,k)=Primitive_VGI(:,i-1,j,k,iGang)
    ...
    subroutine get_facey_first(iMin, iMax, jMin, jMax, kMin, kMax, iBlock)
      ...
      do k=kMin, kMax; do j=jMin, jMax; do i=iMin,iMax
         LeftState_VY(:,i,j,k)=Primitive_VGI(:,i,j-1,k,iGang)
    ...
    subroutine get_facez_first(iMin, iMax, jMin, jMax, kMin, kMax, iBlock)
      ...
      do k=kMin, kMax; do j=jMin, jMax; do i=iMin,iMax
         LeftState_VZ(:,i,j,k)=Primitive_VGI(:,i,j,k-1,iGang)
  ...
  !     do l=lMax,lMin-1,-1
  !       if(all(IsTrueCell_I(l-1:l+1)))then
   !            call muscle_thinc_Ch_limiter(lmin,lmax,Beta,Primitive_VI,dVarLimL_VI,dVarLimR_VI)
  !       else
  !          dVarLimR_VI(:,:) = 0.0
   !         dVarLimL_VI(:,:) = 0.0
   !      ENDIF
  ...
  subroutine limiter(lMin, lMax, Beta, Primitive_VI, dVarLimL_VI, dVarLimR_VI)
    ...
    integer::l
   logical:: useOrigin_mc


   !logical :: useMuscl_Thinc, useCompact, useWENO_Z, useP2T2
   !logical :: adjustDissipation, useUpWind3, useBVD
  ...
  !============================================================================
      ...
      !try to solve the symmetry problem dzy 10/21/2024
   subroutine P2T2_Scheme(lmin,lmax,Beta,Primitive_VI,dVarLimL_VI,dVarLimR_VI) 
      IMPLICIT none
      ...
      real:: FaceL_P2T2(lmin-2:lmax+1),FaceR_P2T2(lmin-2:lmax+1)
  
      real:: FaceL_P2  (lmin-2:lmax+1),FaceR_P2  (lmin-2:lmax+1)
      real:: FaceL_Ts  (lmin-2:lmax+1),FaceR_Ts  (lmin-2:lmax+1)
      real:: FaceL_Tl  (lmin-2:lmax+1),FaceR_Tl  (lmin-2:lmax+1)
  
      real:: Face_L(nVar,lmin-2:lmax+1),Face_R(nvar,lmin-2:lmax+1)
      !notice
   ...
   contains
      ...
      subroutine reconstructL_P2T2(Cellm, Cell,Cellp,&
          FaceL_I_P2, FaceL_I_Ts , FaceL_I_Tl )
  
          real,intent(in):: Cellm, Cell, Cellp
  
          real:: FaceL_I_P2!P2 interpolation
          real:: FaceL_I_Ts!THINC with small 'beta'
          real:: FaceL_I_Tl!THINC with large 'beta'
          real, parameter:: &
              c1 = -1./6., c2 = 5./6., c3 = 2./6.
  
          ! calculate left face first. 
          FaceL_I_P2 = (-1./6.)*Cellm  + (5./6.)*Cell +(2./6.)*Cellp  
          ! Get the 3rd order face value.
          !FaceL_I_P2 = cell
  
          call thinc_L(Cellm,Cell,Cellp,FaceL_I_Ts,beta=1.05)
          
          call thinc_L(Cellm,Cell,Cellp,FaceL_I_Tl,1.2)
  
          
      end subroutine reconstructL_P2T2
      subroutine thinc_L(Cellm,Cell,Cellp,FaceThincL,beta)
         implicit none 
  
         real::Cellm,Cell,Cellp
         real::FaceThincMin,FaceThincMax
         real::FaceThincL
         real::r,cc,A,B
         real::eps=1.0e-20,beta
  
         FaceThincMin = min(Cellm,Cellp)
         FaceThincMax = max(Cellm,Cellp)-FaceThincMin
  
         if ((Cellp-Cellm)>=0.0)then
               r =  1.0
         else 
               r = -1.0
         end if
  
         cc = (Cell-FaceThincMin+eps)/(FaceThincMax+eps)
         B  = exp(r*beta*(2.0*cc-1.0))
         A  = (B/cosh(beta)-1.0)/tanh(beta)
  
         ! Face_L_THINC
         if((Cellp - Cell) * (Cell - Cellm )< 0.0)then
            FaceThincL = Cell
  
         else 
            FaceThincL= FaceThincMin + 0.5 * FaceThincMax * &
            (1.0 + r * (tanh(beta) + A) / (1.0+ A * tanh(beta)))
               
         endif
  
      end subroutine thinc_L
  
      subroutine reconstructR_P2T2(Cellm,Cell,Cellp,&
          FaceR_I_P2, FaceR_I_Ts,FaceR_I_Tl)
  
          real,intent(in)::  Cellm, Cell, Cellp
  
          real:: FaceR_I_P2!P2 interpolation
          real:: FaceR_I_Ts!THINC with small 'beta'
          real::  FaceR_I_Tl!THINC with large 'beta'
          integer:: l
  
          FaceR_I_P2   =  (-1./6.)*Cellp + (5./6.)*Cell + (2./6.)*Cellm
          !FaceR_I_P2   = Cell
  
          call thinc_R(Cellm,Cell,Cellp,FaceR_I_Ts,beta=1.05)
          
          call thinc_R(Cellm,Cell,Cellp,FaceR_I_Tl,1.2)
  
          !==========================
      end subroutine reconstructR_P2T2
      subroutine thinc_R(Cellm,Cell,Cellp,FaceThincR,beta)
          implicit none 
          ...
          FaceThincR = FaceThincMin + 0.5 * FaceThincMax*(1 + r * A)
          endif
  
      end subroutine thinc_R
  
  
  
      subroutine BVD_AD_Algorithm(lmin,lmax,FaceL_P2,FaceR_P2,FaceL_Ts,FaceR_Ts,&
          FaceL_Tl,FaceR_Tl,FaceL_final,FaceR_final)
  
  
       integer::l
       integer,intent(in)::lmin,lmax
       real,intent(in):: FaceL_P2(lmin-2:lmax+1),FaceR_P2(lmin-2:lmax+1)
       real,intent(in):: FaceL_Ts(lmin-2:lmax+1),FaceR_Ts(lmin-2:lmax+1)
       real,intent(in):: FaceL_Tl(lmin-2:lmax+1),FaceR_Tl(lmin-2:lmax+1)
  
       real:: FaceL_I(lmin-2:lmax+1),FaceR_I(lmin-2:lmax+1)
       real:: FaceL_final(lmin-2:lmax+1),FaceR_final(lmin-2:lmax+1)
  
       real :: TBV_1_P2(lMin-1:lMax)
       real :: TBV_2_Ts(lMin-1:lMax)        
       real :: TBV_3_Tl(lMin-1:lMax) 
       real :: TBV_4_new(lMin-1:lMax)
       integer::flag_Func(lMin-1:lMax)
  
       !--------------------------------------------------------
       !STAGE 1.
       !calculate the different TBV of 3 candidates 
       do l = lMin-1,lMax
       TBV_1_P2(l) = dabs(FaceL_P2(l-1 ) - FaceR_P2(l )) + dabs((FaceL_P2(l) - FaceR_P2(l+1)))
       
       TBV_2_Ts(l) = dabs(FaceL_Ts(l-1 ) - FaceR_Ts(l )) + dabs((FaceL_Ts(l) - FaceR_Ts(l+1)))
  
       TBV_3_Tl(l) = dabs(FaceL_Tl(l-1 ) - FaceR_Tl(l )) + dabs((FaceL_Tl(l) - FaceR_Tl(l+1)))
       enddo
       ...
       !initilize
          flag_Func(:)=0 
              !cell index
          do l = lMin-2,lMax+1        
          FaceL_I(l ) = FaceL_P2(l  )
          FaceR_I(l ) = FaceR_P2(l  )
          enddo
  
          do l = lMin-1,lMax!cell_index
              if(TBV_2_Ts(l) < TBV_1_P2(l))then!choose THINC
                  flag_Func(l)=1
  
                  FaceL_I(l) = FaceL_Ts(l)
                  FaceR_I(l) = FaceR_Ts(l)
              else
                  flag_Func(l)=0
          ...
          do l = lMin-1,lMax!cell_index
          if(flag_Func(l) ==1)then!all 3 Cells choose THINC
                  FaceL_I(l-1) = FaceL_Ts(l-1)
                  FaceR_I(l-1) = FaceR_Ts(l-1)!cell i-1
          ...
          !======== dissipation adjusting process   ======
  
          !With λ decreased from 1.0 to 0.5, the interpolation becomes more centralized.
          ! With this parameter, the dissipation property can be effectively adjusted 
          ! between high order upwind scheme and non-dissipative central scheme.
          if(adjustDissipation)then
          do l = lMin-1,lMax!
                  !if界面两侧都是光滑区域,就调整耗散
              if(FaceL_I(l) == FaceL_P2(l).and.FaceR_I(l+1) == FaceR_P2(l+1))then
  
                  FaceL_I(l  )   = ADlamda*FaceL_P2(l  )+(1.0-ADlamda)*FaceR_P2(l+1)
                  FaceR_I(l +1 ) = ADlamda*FaceR_P2(l+1)+(1.0-ADlamda)*FaceL_P2(l  )
              endif
          ...
          !UPDATE TBV_
          do l = lMin-1,lMax!cell_index
          TBV_4_new(l) = dabs(FaceL_I(l-1) - FaceR_I(l)) + dabs((FaceL_I(l) - FaceR_I(l+1)))
          enddo
          ...
          do l = lMin-1,lMax
          if(TBV_3_Tl(l) < TBV_4_new(l))then 
                  flag_Func(l)=2
                      
                  FaceL_final(l  ) = FaceL_Tl(l )                                    
                  FaceR_final(l  ) = FaceR_Tl(l )
          ELSE
                  FaceL_final(l  ) =  FaceL_I(l  )  
                  FaceR_final(l  ) =  FaceR_I(l  ) 
      ...
      END subroutine BVD_AD_Algorithm
